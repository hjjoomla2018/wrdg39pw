...
class DJClassifiedsNotify {
	...
	public static function messageAskProfileFormNotification($profile,$author,$message,$files,$replyto,$replytoname,$custom_fields_msg){
		$app 	= JFactory::getApplication();
		$config = JFactory::getConfig();
		$par 	= JComponentHelper::getParams( 'com_djclassifieds' );
		$db 	= JFactory::getDBO();
		...
		$reciver = $profile['user']->id;
		
		$mailfrom = $app->getCfg( 'mailfrom' );
		$fromname = $config->get('sitename');
			
		$subject = self::parseMessageBody($email->title,$email->id,'',$reciver,'','','','',$message,$author,'','','','',$custom_fields_msg);
		$message = self::parseMessageBody($email->content,$email->id,'',$reciver,'','','','',$message,$author,'','','','',$custom_fields_msg);
	...
	public static function notifyBuynowBuyer($id,$buyer,$quantity,$opt_name){
		...
		if($pd_field_id_c){
			$query = "SELECT p.value FROM #__djcf_fields_values_profile p "
					."WHERE p.field_id = '".$pd_field_id_c."' AND p.user_id  = ".$item->user_id." ";
			$db->setQuery($query);
			$item->purchase_details=$db->loadResult();
	...
	public static function messageProfileAskFormContact($profile,$author,$message,$files,$replyto,$replytoname,$custom_fields_msg){
		...
		JPluginHelper::importPlugin('djclassifiedsmessage');
		$dispatcher->trigger('onDJClassifiedsSendProfileMessage', array($profile,$author,$mailto,$mailfrom,$fromname,$replyto,$replytoname,$subject,$message,$files,$custom_fields_msg));
