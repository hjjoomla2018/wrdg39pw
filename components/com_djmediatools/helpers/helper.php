...
class DJMediatoolsLayoutHelper extends JObject
{
	...
	public function getSlides(&$params) {
		...
		if(is_array($slides) && count($slides)>0) foreach($slides as $key => $slide) {
			...
			$path = (strcasecmp(substr($slide->image, 0, 4), 'http') != 0 && !empty($slide->image)) ? JPath::clean(JPATH_ROOT . DS . str_replace('/', DS, $slide->image)) : $slide->image;
			...
			$slide->blank = "data:image/svg+xml;charset=utf-8,%3Csvg xmlns%3D'http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg' width%3D'".$slide->size->w."' height%3D'".$slide->size->h."' viewBox%3D'0 0 ".$slide->size->w." ".$slide->size->h."'%2F%3E";
		...
		if($video) {
			...
			$doc->addScript(JURI::root(true).'/components/com_djmediatools/assets/js/video.js?v='.self::$_version, 'text/javascript', self::canDefer());
		}
	...
	public function addScripts(&$params) {
		...
		$document->addScript(JURI::root(true).'/media/djextensions/picturefill/picturefill.min.js', 'text/javascript', self::canDefer());
		if($jquery) $document->addScript(JURI::root(true).'/media/djextensions/jquery-easing/jquery.easing.min.js', 'text/javascript', self::canDefer());
		else $document->addScript(JURI::root(true).'/components/com_djmediatools/assets/js/powertools-1.2.0.js', 'text/javascript', self::canDefer());
		$document->addScript(JURI::root(true).'/components/com_djmediatools/layouts/slideshow/js/'.(!$jquery ? 'moo.':'').'slideshow.js?v='.self::$_version, 'text/javascript', self::canDefer());
		...
		if(JFile::exists(JPath::clean(JPATH_ROOT.DS.$script))) {
			$document->addScript(JURI::root(true).'/'.$script.'?v='.self::$_version, 'text/javascript', self::canDefer());
		}
	...
	public function addLightbox($type, $popup = false) {
		...
		switch($type) {
			case 'magnific':
				...
				$document->addScript(JURI::root(true).'/media/djextensions/magnific/magnific.js', 'text/javascript', self::canDefer());
				$document->addScript(JURI::root(true).'/components/com_djmediatools/assets/js/magnific-init.js', 'text/javascript', self::canDefer());
				if($popup) {
					// item details in popup
					$document->addStyleSheet(JURI::root(true).'/components/com_djmediatools/assets/css/slide-popup.css');
					$document->addScript(JURI::root(true).'/components/com_djmediatools/assets/js/album.js?v='.self::$_version, 'text/javascript', self::canDefer());
				}
				break;
			case 'photoswipe':
				JHTML::_('jquery.framework');
				...
				$document->addScript(JURI::root(true).'/media/djextensions/photoswipe/photoswipe.min.js', 'text/javascript', self::canDefer());
				$document->addScript(JURI::root(true).'/media/djextensions/photoswipe/photoswipe-ui-default.min.js', 'text/javascript', self::canDefer());
				$document->addScript(JURI::root(true).'/components/com_djmediatools/assets/js/photoswipe-init.js', 'text/javascript', self::canDefer());
				break;
			case 'picbox':
				JHTML::_('behavior.framework');
				$document->addStyleSheet(JURI::root(true).'/components/com_djmediatools/assets/picbox/css/picbox.css');
				$document->addScript(JURI::root(true).'/components/com_djmediatools/assets/picbox/js/picbox.js', 'text/javascript', self::canDefer());
				break;
			case 'slimbox':
				JHTML::_('behavior.framework');
				$document->addStyleSheet(JURI::root(true).'/components/com_djmediatools/assets/slimbox-1.8/css/slimbox.css');
				$document->addScript(JURI::root(true).'/components/com_djmediatools/assets/slimbox-1.8/js/slimbox.js', 'text/javascript', self::canDefer());
				break;
			default:
				return;
		}
