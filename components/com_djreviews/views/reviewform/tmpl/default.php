...
foreach ($this->userRating as $userRate) {
	if ($userRate->avg_rate > 0 && (int)$this->item->id != $userRate->id && (int)$params->get('multiple_rating', 0) == 0) {
		$canRate = false;
	}
}
...
<form action="<?php echo JRoute::_('index.php?option=com_djreviews'); /*(string)JUri::getInstance();*/ ?>" method="post" name="djrv_review_form" id="djrv_review_form-<?php echo (int)$this->form->getValue('item_id'); ?>" class="form-validate djrv_review_form">
		...
		<div class="span12 form-horizontal">
			...
			<?php if ($user->guest) { ?>
				...
				<?php if ($captcha) {?>
					<div class="control-group">
						<div class="controls">
							<div id="djrv_captcha-<?php echo (int)$this->form->getValue('item_id'); ?>" class="djrv_captcha" data-sitekey="<?php echo $captcha; ?>" data-theme="light" data-size="compact"></div>
						</div>
					</div>
				<?php } ?>
			<?php } ?>
					...
					<div class="djrv_rating_field">
						...
						<?php } else {?>
							...
							<div class="control-group">
								<div class="control-label">
									<?php if ($field->description) { ?>
										<label class="djrv_field_name djrv_tooltip" title="<?php echo $field->label; ?>" data-content="<?php echo htmlspecialchars($field->description); ?>"><?php echo $field->label; ?><?php if ($field->required == 1) echo '&nbsp;<sup>*</sup>';?></label>
									<?php } else {?>
										<label class="djrv_field_name"><?php echo $field->label; ?><?php if ($field->required == 1) echo '&nbsp;<sup>*</sup>';?></label>
									<?php } ?>
								</div>
							<div class="controls">
		...
		<fieldset>
			<div class="control-group">
				<div class="controls">
					...
					<button type="button" class="djrv_close_form_button btn button"><?php echo JText::_('COM_DJREVIEWS_CLOSE')?></button>
				</div>
			</div>
		</fieldset>
		</div>
	...
	<?php if (!empty($custom)) {
		foreach($custom as $k=>$v) {
			echo '<input type="hidden" name="custom['.$k.']" value="'.htmlspecialchars($v).'" />';
		}
	} ?>
