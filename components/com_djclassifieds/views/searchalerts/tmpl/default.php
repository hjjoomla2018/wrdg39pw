		...
		<div class="dj-users_search_alerts">		
			<div class="dj-items-table2">	
				...
				foreach($this->items as $i){
					...
					echo '<div class="item_row row'.$row.'">';
						...
						echo '<div class="item_col">';
							...
							if(isset($i->params->uid) && $i->params->uid){
								$se_user = JFactory::getUser($i->params->uid);
								if($par->get('authorname','name')=='name'){										
									$u_name = $se_user->name;
								}else{
									$u_name = $se_user->username;
								}	
								echo '<div class="col_row"><span class="col_row_label">'.JText::_('COM_DJCLASSIFIEDS_PROFILE').':</span><span class="col_row_val">'.$u_name.'</span></div>';
							}
				...
				if(count($this->items)==0){
					...
					echo JText::_('COM_DJCLASSIFIEDS_NO_SEARCH_ALERTS');
					echo '</div>';
				}
