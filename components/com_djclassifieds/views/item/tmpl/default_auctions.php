...
<?php if($par->get('auction_timer','0')){ ?>
	...
	<script>
		...
		window.addEvent('domready', function(){
			...
			var server_time = new Date('<?php echo date("Y/m/d H:i:s"); ?>').getTime();
			var browser_time = new Date().getTime();
			var time_offset = server_time - browser_time;
			
			if(document.getElementById('djtimer')){
				//var countDownDate = new Date('<?php echo $item->date_exp; ?>').getTime();
				var countDownDate = new Date('<?php echo date_format(date_create_from_format('Y-m-d H:i:s', $item->date_exp), 'Y/m/d H:i:s'); ?>').getTime(); 
