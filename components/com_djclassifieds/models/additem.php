	...
	function checkAdsLimits(){
		...
		if($id==0){
			...
			if($par->get('advert_limit_5min',5)>0){
				$date_last5 = date('Y-m-d H:i:s',mktime(date("H"), date("i")-5, date("s"), date("m"), date("d"),date("Y")));
				if($user->id>0){
					$query = "SELECT COUNT(id) FROM #__djcf_items i "
							."WHERE i.user_id = ".$user->id." AND i.date_start >= '".$date_last5."'";					
				}else{					
					$query = "SELECT COUNT(id) FROM #__djcf_items i "
							."WHERE i.ip_address = '".$user_ip."' AND i.date_start >= '".$date_last5."'";
				} 
																
				$db->setQuery($query);
				$check = $db->loadResult();
				if($check>=$par->get('advert_limit_5min',5)){
					$msg = JText::_('COM_DJCLASSIFIEDS_ADVERTS_LIMIT_LAST_5MIN_ALERT');
			...
			if($par->get('advert_limit_hour',10)>0){
				...
				if($user->id>0){
					$query = "SELECT COUNT(id) FROM #__djcf_items i "
							."WHERE i.user_id = ".$user->id." AND i.date_start >= '".$date_lasth."'";
				}else{
					$query = "SELECT COUNT(id) FROM #__djcf_items i "
							."WHERE i.ip_address = '".$user_ip."' AND i.date_start >= '".$date_lasth."'";
				}
			
				$db->setQuery($query);
				$check = $db->loadResult();
				if($check>=$par->get('advert_limit_hour',10)){
					$msg = JText::_('COM_DJCLASSIFIEDS_ADVERTS_LIMIT_LAST_HOUR_ALERT');
					$app->redirect($link,$msg,'error');
				}
			}
			
			if($par->get('advert_limit_day',50)>0){
				$date_lastd = date('Y-m-d H:i:s',mktime(date("H"), date("i"), date("s"), date("m"), date("d")-1,date("Y")));
				
				if($user->id>0){
					$query = "SELECT COUNT(id) FROM #__djcf_items i "
							."WHERE i.user_id = ".$user->id." AND i.date_start >= '".$date_lastd."'";
				}else{
					$query = "SELECT COUNT(id) FROM #__djcf_items i "
							."WHERE i.ip_address = '".$user_ip."' AND i.date_start >= '".$date_lastd."'";
				}
					
				$db->setQuery($query);
				$check = $db->loadResult(); 				
				if($check>=$par->get('advert_limit_day',50)){
					$msg = JText::_('COM_DJCLASSIFIEDS_ADVERTS_LIMIT_LAST_DAY_ALERT');
					$app->redirect($link,$msg,'error');
				}
			}
