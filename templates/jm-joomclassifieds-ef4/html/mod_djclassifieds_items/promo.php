...
?>
<div class="mod_djclassifieds_items promo clearfix"  id="mod_djclassifieds_items<?php echo $module->id; ?>">
		...
		<?php
			...
			foreach($items as $i){
				...
				if($params->get('show_img')==1){
					...
					$col1_width = ($params->get('img_width','') ? 'width:'.$params->get('img_width','').'px;' : '');

					echo '<div class="img-wrap" style="'.$col1_width.'">';
					if(count($i->images)){
				...
				$col2_width = ( $params->get('show_img')==1 && !empty($params->get('img_width')) ) ? 'width: calc(100% - '.$params->get('img_width').'px); ' : '';

				echo '<div class="promo-left" style="'.$col2_width.'">';


				if(($params->get('show_title','1')==1) || ($params->get('show_date')==1) || ($params->get('show_cat')==1) || ($params->get('show_price')==1) || ($params->get('show_type','1')) || ($params->get('show_region','1'))){
									...
									<span class="row_value<?php if($f->hide_on_start){echo ' djsvoc" title="'.htmlentities($f->value); }?>" rel="<?php echo $tel_tag; ?>" >
										...
										if($f->type=='textarea'){
											...
											if($f->value==''){echo '---'; }
												...
												if($cfpar->get('cf_values_to_labels','0') && $f->type!='inputbox' && $f->type!='textarea'){
				...
				$redmore_lable = ( JText::_('COM_DJCLASSIFIEDS_CHECK_OFFER') != 'COM_DJCLASSIFIEDS_CHECK_OFFER' ) ? JText::_('COM_DJCLASSIFIEDS_CHECK_OFFER') : 'Check offer';
				echo '<div class="jm-buttons"><a class="readmore2" href="'.JRoute::_(DJClassifiedsSEO::getItemRoute($i->id.':'.$i->alias,$i->cat_id.':'.$i->c_alias,$i->region_id.':'.$i->r_name)).'">'.$redmore_lable.'</a></div>';
